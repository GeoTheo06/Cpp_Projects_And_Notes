...
void meFilo(int arithmosPaiktwn, int pontoiMeFilous, int pontoiSwsthsLekshs, int pontoiLathosLekshs, int prospatheies) { //kane na leei ston plhthintiko h ston eniko opote xreiazetai kai ftiakse to arxeio //???? ?? txt ?????? ??????????????? ?? +5 (?? ??? ????? ?????????)
