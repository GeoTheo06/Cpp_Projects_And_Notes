...
void apokryptografhsh() {
	...
	//briskw tous arithmous pou prostethikan sthn kathe leksh (den to exw testarei)
	int* additionalNumbers = new int;
	for (int i = 0; i < theshDeuterouKenou - 2; i++) {
		if (pointers[i] > xarakthrasKwdikopoihshs) {
			additionalNumbers[i] = pointers[i] - xarakthrasKwdikopoihshs; //an einai megalytero den exei meiwthei kata 94
		} else if (pointers[i] < xarakthrasKwdikopoihshs) {
			pointers[i] += 94;
			additionalNumbers[i] = pointers[i] - xarakthrasKwdikopoihshs; //an den einai megalytero shmainei oti exei meiwuei kata 94 kai prepei na to aukshsw
		}
	}
}
