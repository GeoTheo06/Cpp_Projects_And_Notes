...
void kryptografhsh() {
	...
	delete[] textNum;
	delete[] privateKey;
	delete[] additionNumbers;
	delete[] kryptografhmenoKeimeno;
...
void apokryptografhsh() {
	...
	for (int i = theshDeuterouKenou + 1; i < textLength; i++) {
		if (kryptografhmenoKeimenoChar[i] == 32) {
			w += 1;
			apokryptografhmenoKeimenoInt[i] = 32;
		} else {
			if (kryptografhmenoKeimenoChar[i] > xarakthrasKwdikopoihshs) {
				apokryptografhmenoKeimenoInt[i] = kryptografhmenoKeimenoChar[i] - additionNumbers[w]; //an einai megalytero den exei meiwthei kata 94
			}
			if (kryptografhmenoKeimenoChar[i] < xarakthrasKwdikopoihshs) {
				apokryptografhmenoKeimenoInt[i] = kryptografhmenoKeimenoChar[i] - additionNumbers[w] + 94; //an den einai megalytero shmainei oti exei meiwuei kata 94 kai prepei na to aukshsw
			}
		}
		apokryptografhmenoKeimeno[i] = apokryptografhmenoKeimenoInt[i];
		cout << apokryptografhmenoKeimeno[i];
	}

	delete[] kryptografhmenoKeimenoChar;
	delete[] pointers;
	delete (additionNumbers);
	delete[] apokryptografhmenoKeimeno;
	delete[] apokryptografhmenoKeimenoInt;
	cout << endl;
}
...
//gia kapoio logo to g doulevei kalutera apo ola