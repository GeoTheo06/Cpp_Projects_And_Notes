...
void apokryptografhsh() {
	...
	int w = 1;
	char* apokryptografhmenoKeimeno = new char;
	int* apokryptografhmenoKeimenoInt = new int;
	for (int i = theshDeuterouKenou + 1; i < textLength; i++) {
		if (kryptografhmenoKeimenoChar[i] == 32) {
			...
			if (kryptografhmenoKeimenoChar[i] < xarakthrasKwdikopoihshs) {
				apokryptografhmenoKeimenoInt[i] = kryptografhmenoKeimenoChar[i] - xarakthrasKwdikopoihshs + 94; //an den einai megalytero shmainei oti exei meiwuei kata 94 kai prepei na to aukshsw
			}
		}
