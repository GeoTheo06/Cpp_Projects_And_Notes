...
void nikh() {
	cout << endl << endl << "Sygxarhthria! Ta kataferes!" << endl << "Exeis " << "pontous";
}
...
void monos1() {
	...
	while (prospatheies != 0) {
		...
		bool equal = (randomLekshChar == lekshPouThaTypwthei);
		for (int i = 0; i < textLength; i++) {

			if (equal) {
				return;//prepei na to testareis kai na breis tropo na ftiakseis th bathmologia
			}
		}
		...
		for (int i = 0; i <= 23; i++) {
			cout << "lathos grammata: " << lathosGrammataXrhsth[i];
		}
