...
void enemyAttack() {
	...
	if (usingShield) {
		...
		if (geo.shield >= 0) {
			...
			Sleep(2000);
			geo.health = geo.health + geo.shield;
	...
	for (int i = 1; i < rounds; i++) { //it runs for every round (Game Loop)
		if (geo.mana >= 5) { // It could be 0 but if the mana is 4, 3, 2 or 1 , the game would continue, though the player would not be able to execute moves as the minimum mana for pepper spray is 5
			...
			while (!playerWin && geo.health > 0) {
				...
				Sleep(500);
				cout << "1. attack (Basic attack: " << geo.attack << " Dmg)" << endl;
				Sleep(500);
				cout << "2. heal (Gain " << maxHealth * 0.30 << " HP & " << maxMana * 0.20 << " M)" << endl;
				Sleep(500);
				if (geo.shield > 0) {
					cout << "3. Use shield (Absorb " << geo.shield << " Dmg + " << maxHealth * 0.10 << " HP + chance of deflecting enemy's attack)" << endl << endl;
				} else {
					cout << "Your shield is broken" << endl << endl;
				}
