...
int playerAttack() { //if I name this just "attack" it will interfere with the variable attack of type float, declared at line 18
	cout << "You have 3 attack choices:" << endl << "1. pepper Spray (P: -5M) (E: -attack * 0.25 = " << geo.attack * 0.25 << ")" << endl << "2. Sword (P: -15M) (E: -" << geo.attack << ")" << endl << "3. Punch the earth so hard that you create a huge crack on the surface and the enemy dies from fall damage (P: -125M) (E:DEAD)" << endl << endl;
			...
			if (geo.mana >= 5) { // if mana is 5 or below 5. (It could be 0 but if the mana is 4, 3, 2 or 1 , the game would continue, though the player would not be able to execute moves)
				...
				while (wrongMoveInput) {
					...
					if (moveInput == 1) {
						...
						while (geo.mana >= 5 && playerAttack == 0) {
							playerAttack();
						}
