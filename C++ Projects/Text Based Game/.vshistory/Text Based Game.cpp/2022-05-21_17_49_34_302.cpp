...
Player geo(0, 0, 0, 0);
Enemy enemy(0, 0); //initializing class Player and Enemy here so they are global and can be accessed easier
...
class Player { //This class kinda works like storage
	...
	int choiceAttack() { //if I name this just "attack" it will interfere with the variable attack of type float, declared at line 18
		...
		if (enemy.health > 0) {
			cout << "Enemy is alive and has " << enemy.health << " health left." << endl;
			return 0;
		} else {
			...
			if (geo.mana >= 5) { // if mana is 5 or below 5. (It could be 0 but if the mana is 4, 3, 2 or 1 , the game would continue, though the player would not be able to execute moves)
				...
				while (wrongMoveInput) {
					...
					if (moveInput == 1) {
						int getEnemyState = 0;
						do {
							geo.choiceAttack(enemy.health);
							int getEnemyState {geo.choiceAttack(enemy.health)};
						...
						//to oti eftase ws edw shmainei oti nikhse opote afkshse ta stats tou paikth
						wrongMoveInput = false;

					} else if (moveInput == 2) {
						geo.choiceHeal(geo.health);
						wrongMoveInput = false;
					} else {
						cout << "Wrong input. Try again..." << endl;
						wrongMoveInput = true;
