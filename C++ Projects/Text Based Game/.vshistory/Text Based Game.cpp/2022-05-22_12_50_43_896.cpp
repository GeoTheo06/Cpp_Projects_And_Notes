...
bool usingShield = false; // thymisou na thn kaneis ksana false afou thn xrhsimopoihseis
string name = "";
...
void playerAttack() { //if I name this just "attack" it will interfere with the variable attack of type float, declared at line 18
	...
	if (enemy.health > 0) {
		...
		cout << endl << endl << "Congratulations! You killed " << name << "! Prepare for the next level..." << endl << endl;
		Sleep(3000);
		playerWin = true;
	}
}
...
void enemyAttack() {
	...
	if (usingShield) {
		...
		if (playerShieldLeft >= 0) {
			cout << name << " dealt " << enemy.attack << " damage on the shield and didn't afect your health" << endl;
			Sleep(1000);
			...
			Sleep(2000);
			cout << "You lost " << playerShieldLeft * -1 << " HP"; // it is multiplied by -1 so it becomes positive
...
void playerHeal(float maxPlayerHealth, float maxPlayerMana) {
	geo.health = geo.health + (maxPlayerHealth * 0.20);
	geo.mana = geo.mana + (maxPlayerMana * 0.20);
}
...
int main() {
	...
	float maxHealth = geo.health;
	float maxMana = geo.mana;
