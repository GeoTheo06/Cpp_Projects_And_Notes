...
int playerAttack() { //if I name this just "attack" it will interfere with the variable attack of type float, declared at line 18
	cout << "You have 3 attack choices:" << endl << "1. pepper Spray (P: -5M) (E: -attack * 0.25 = " << geo.attack * 0.25 << ")" << endl << "2. Sword (P: -15M) (E: -" << geo.attack << ")" << endl << "3. Punch the earth so hard that you create a huge crack on the surface and the enemy surely from fall damage (P: -125M) (E:DEAD)" << endl << endl;
	...
	do {
		...
		if (attackChoice == 1) {
			geo.mana -= 5;
			geo.health = geo.health - (geo.attack * 0.25);
			wrongInput = false;
	...
	if (geo.health > 0) {
		cout << "Enemy is alive and has " << geo.health << " HP left." << endl << endl;
		Sleep(3000);
		return 0;
	} else {
		cout << "Congratulations! You killed the enemy! Prepare for the next level..." << endl << endl;
		Sleep(3000);
		return 1;
	}
}
...
int main() {
	...
	do {
		cout << "Select difficulty: 1. Easy " << endl << "2. Medium" << endl << "3. Hard" << endl << "4. random" << endl << "5. You choose" << endl;
		int choice = 0;
		cin >> choice;
		if (choice == 5) {
			...
			while (geo.health + geo.mana + geo.attack + geo.armor != points) {
				...
				cout << "There are " << remainingPoints << " Points left which will be used for your armor." << endl << endl;
			...
			if (geo.mana >= 5) { // if mana is 5 or below 5. (It could be 0 but if the mana is 4, 3, 2 or 1 , the game would continue, though the player would not be able to execute moves)
				...
				while (wrongMoveInput) {
					...
					if (moveInput == 1) {
						...
						do {
							playerAttack();
							int getEnemyState {playerAttack()};
