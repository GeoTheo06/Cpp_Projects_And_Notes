...
void kryptografhsh() {
	...
	for (int i = 0; i < lexeis; i++) {
		...
		if (numbersOfPrivateKey[i] > 126) {
			numbersOfPrivateKey[i] -= 126; //(to 94 bgainei apo to 126-32)
...
void apokryptografhsh() {
	...
	for (int i = 0; i < theshDeuterouKenou - 2; i++) {
		...
		if (pointers[i] < xarakthrasKwdikopoihshs) {
			additionNumbers[i] = 126 - xarakthrasKwdikopoihshs;
			additionNumbers[i] = pointers[i] - xarakthrasKwdikopoihshs; //an den einai megalytero shmainei oti exei meiwuei kata 94 kai prepei na to aukshsw
		}
		cout << additionNumbers[i];
	}
}
