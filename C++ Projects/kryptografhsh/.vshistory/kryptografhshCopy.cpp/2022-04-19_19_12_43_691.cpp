...
void apokryptografhsh() {
	...
	for (int i = theshDeuterouKenou + 1; i < textLength; i++) {
		if (kryptografhmenoKeimenoChar[i] == 32) {
			...
			if (kryptografhmenoKeimenoChar[i] < xarakthrasKwdikopoihshs) {
				apokryptografhmenoKeimenoInt[i] = kryptografhmenoKeimenoChar[i] + 94 - xarakthrasKwdikopoihshs; //an den einai megalytero shmainei oti exei meiwuei kata 94 kai prepei na to aukshsw
			}
		}
